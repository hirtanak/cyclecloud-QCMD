...
SW=lammps-rhel8
echo "starting 51.install-${SW}.sh"
  ...
  Default )
    ...
    if [ -z ${KOKKOS} ]; then      
      ...
      if [[ ! -f ${HOMEDIR}/${LAMMPS_DIR}/bin/lmp ]]; then
        ...
        # default
        #/usr/local/bin/cmake ${HOMEDIR}/${LAMMPS_DIR}/cmake -DBUILD_MPI=on -DBUILD_OMP=on -DCMAKE_C_COMPILER=/opt/${OPENMPI_PATH}/bin/mpicc \
	#	-DCMAKE_CXX_COMPILER=/opt/${OPENMPI_PATH}/bin/mpicxx -DCMAKE_Fortran_COMPILER=mpif90 -DPKG_MOLECULE=on -DPKG_KSPACE=on \
	#	-DMULTITHREADED_BUILD=${CORES}
        # custom
	...
	set +ue
	CMD=$(file /opt/intel/oneapi/mpi/latest/bin)
	if [[ -n "$CMD" ]]; then
	  IMPI_PATH=/opt/intel/oneapi/mpi/latest/bin
	else
	...
	MPI_ROOT=$IMPI_PATH
        ...
        # check lmp-intel -f -e does not work
      	if [[ ! -f ${HOMEDIR}/${LAMMPS_DIR}-intel/bin/lmp-intel ]]; then
	  make -C ${HOMEDIR}/${LAMMPS_DIR}-intel clean | exit 0
          # intel mpi build
	  ...
	  cp ${CYCLECLOUD_SPEC_PATH}/files/oneapi.repo /etc/yum.repos.d/ && true
          time dnf install -y -q intel-basekit
	  time dnf install -y -q intel-hpckit
	  ICOMP_PATH=/opt/intel/oneapi/compiler/latest/linux/bin

	  time CMAKE_ROOT=${HOMEDIR}/CMake-3.23.2/bin/cmake ${HOMEDIR}/CMake-3.23.2/bin/cmake ${HOMEDIR}/${LAMMPS_DIR}-intel/cmake/ \
		  -DCMAKE_C_COMPILER=$ICOMP_PATH/icx -DCMAKE_CXX_COMPILER=$ICOMP_PATH/icpx -DBUILD_MPI=on -DBUILD_OMP=on \
		  -DPKG_MOLECULE=on -DPKG_KSPACE=on -DPKG_REAXFF=on
          time make -j ${CORES}
...
cp /opt/cycle/jetpack/logs/cluster-init/QCMD/execute/scripts/51.install-${SW}.sh.out ${HOMEDIR}/logs/
chown ${CUSER}:${CUSER} ${HOMEDIR}/logs/51.install-${SW}.sh.out
